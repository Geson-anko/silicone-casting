...
- Surface Cut・Air Vents・キー配置の評価メッシュを `SurfaceSnapshot` に統一し、描画中の一時データ・タイマー・表示ハンドラーを `DrawingSession` で管理

- キー配置をネイティブツールと `KeyPair` に統一し、旧 Preview / Commit / Cancel と内部 Add / Move オペレータを削除

- 配合の保存先を `Scene.silicone_casting.mixture` へ整理し、レシピ JSON を version 2 へ更新。旧配合保存値の自動移行は行わず、version 1 の JSON は入力を変更せずに拒否
...
- 描画中にプレビューや対象メッシュを外部から削除しても、取消時に一時データと描画状態を解放し、次の描画を開始できるように修正
