...
Base Volume の直接変更と Use Mixture Total は、全染料の滴数を新体積／旧体積で
スケールする。無効行と小数滴も対象とし、校正濃度は変えない。
既存 `.blend` の体積保存キーを維持し、読み込み後の最初の編集でも同比率を適用する。

Export JSON は全プロファイルの名前・体積・ベース色・透明度・全染料入力を保存する。
染料色は Hue と Lightness を保存し、白・黒でも Hue を保持する。マテリアル参照は含めない。
Import JSON は全内容を検証後、既存プロファイルへ追加し、新しい専用マテリアルを作成する。
同名も独立したプロファイルとして追加する。体積を先に復元し、滴数を二重スケールしない。
形式は `format: silicone_casting`、`version: 1`、`kind: COLORS`、`data` の JSON とする。
...
実測スペクトルによるKubelka–Munk、多点校正、プロファイル複製、染料行の並べ替えは扱わない。
